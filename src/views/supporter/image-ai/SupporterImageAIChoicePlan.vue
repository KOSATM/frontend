  ...
  <div class="p-4">
    ...
    <div class="card p-4 mt-3">
      ...
      <div class="plans-accordion">
        <div
          v-for="(day, dIdx) in plansDays"
          :key="day.id"
          :class="['day-block card mb-3', { expanded: expandedDay === dIdx }]"
        >
          <div
            class="day-header d-flex align-items-center p-3"
            :class="{ 'day-header-active': expandedDay === dIdx }"
            @click="toggleDay(dIdx)"
            role="button"
            tabindex="0"
          >
            <div>
          ...
          <div v-show="expandedDay === dIdx" class="day-body p-2">
            <ul class="list-unstyled m-0">
              ...
              >
                <template v-if="!day._topItem">
                  ...
                  <div v-if="mode === 'add'" class="action-wrap">
                    <button
                      class="btn btn-add"
                      @click.stop="addToItinerary(dIdx, -1)"
                      :disabled="alreadyAddedAnywhere()"
                      :title="alreadyAddedAnywhere() ? '이미 추가된 장소입니다' : 'Add to top of day'"
                    >＋</button>
                  </div>
                </template>
              ...
              <li
                v-for="(it, iIdx) in day.items"
                :key="it.id"
                class="day-item d-flex align-items-center p-3"
                :class="{
              ...
              >
                ...
                <div v-else-if="mode === 'add'" class="action-wrap">
                  <button
                    class="btn btn-add"
                    @click.stop="addToItinerary(dIdx, iIdx)"
                    :disabled="alreadyAddedAnywhere()"
                    :title="alreadyAddedAnywhere() ? '이미 추가된 장소입니다' : 'Add here'"
                  >＋</button>
                </div>
                ...
                <div v-else-if="mode === 'replace'" class="action-wrap">
                  <button
                    ...
                    :class="{ active: it._replaced }"
                    @click.stop="replaceThis(dIdx, iIdx)"
...
<style scoped>
.card { border-radius: 12px; position: relative; }

/* day block */
.day-block { border: 1px solid #f3e8ff; border-radius: 12px; overflow: hidden; background: #fff; }
.day-block.expanded { box-shadow: 0 12px 30px rgba(27,59,111,0.06); }

/* day header */
.day-header { background: #fff; cursor: pointer; transition: background .18s, color .18s; }
.day-header .day-label { color: #ff8c00; font-weight:700; font-size:13px; }
.day-header .day-title { color: #1b3b6f; font-size:16px; }
.day-header .day-date { color: #6b7280; }
...
.day-block.expanded .text-accent { color: #FFF; }

/* change chevron color on expanded */
.day-block.expanded .chev { color: rgba(255,255,255,0.95); }

/* day body */
.day-body { background: #fff9ff; transition: background .18s; }
...
.day-item:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(27,59,111,0.04); }

/* top-add row: 조금 더 연하게 표시해 자연스럽게 보이도록 함 */
.day-item-top { background: transparent; border-top: none; }
.day-item-top .fw-medium { color: #6b7280; font-weight:600; }
...
/* visual states */
.item-added { border-left: 4px solid #10b981; background: #f1fff6; box-shadow: 0 6px 18px rgba(16,185,129,0.04); }
.item-selected { border-color: #1b3b6f; background: #f3f7ff; box-shadow: 0 8px 20px rgba(27,59,111,0.06); }
...
.btn-undo {
  width:36px; height:36px; border-radius:8px; border:1px solid #e6e6ea; background:#fff; color:#1b3b6f; cursor:pointer; margin-left:6px;
}
.btn-undo:hover { background:#f3f7ff; }
...
.btn-remove {
  width:36px; height:36px; border-radius:8px; border:1px solid #f0e6ea;
  background: #fff; color:#ef4444; font-weight:700; cursor:pointer;
}
.btn-remove:hover { background:#fff7f7; }

/* action buttons */
.action-wrap { width: 64px; display:flex; align-items:center; justify-content:center; }
.btn-add {
  width:36px; height:36px; border-radius:8px; border:none; background:#fff; color:#1b3b6f; font-weight:700; font-size:18px; box-shadow: 0 4px 10px rgba(27,59,111,0.06); cursor:pointer;
}
.btn-add:disabled { opacity:0.45; cursor:not-allowed; color:#94a3b8; }
.btn-replace {
  width:36px; height:36px; border-radius:8px; border:1px solid #e6e6ea; background:#fff; color:#6b7280; font-weight:700; font-size:14px; cursor:pointer;
}
.btn-replace.active { background:#1b3b6f; color:#fff; border-color:#1b3b6f; box-shadow: 0 6px 18px rgba(27,59,111,0.12); }
...
/* utilities */
.text-accent { color: #1b3b6f; font-weight:700; }
